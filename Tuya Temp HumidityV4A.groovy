...
//        command "initialize", [[name: "Manually initialize the device after switching drivers.  \n\r     ***** Will load device default values! *****" ]]
    ...
    preferences {
        ...
        input (name: "logTempHumi", type: "bool", title: "Log raw Temperature and Humidity Reports", description: "<i>Display measured values in HE log page. Recommended value is <b>false</b></i>", defaultValue: false)
        ...
        input (name: "advancedOptions", type: "bool", title: "Advanced options", description: "May not be supported by all devices!", defaultValue: true)
...
//              		log.debug 'Humi event '+ fncmd
            ...
            case 0x13 : // (19) temperature sensitivity(value/2/10) default 0.3C ( divide / 2 for Haozee only?)
                if ((safeToDouble(settings?.temperatureSensitivity)*10 as int) == (fncmd as int)) {
                    if (settings?.txtEnable) log.info "${device.displayName} reported temperature sensitivity ${fncmd/10} C"
                }
                else {
                    if (settings?.txtEnable) log.warn "${device.displayName} warning: temperature sensitivity reported by the device (${fncmd/10}) differs from the preference setting (${settings?.temperatureSensitivity})"
                }
                break
...
def temperatureEvent( temperature, isDigital=false ) {
	if (settings?.logTempHumi) log.debug 'Temperature Event ' +temperature
    def map = [:]
...
//    map.descriptionText = "${map.name} is ${map.value} ${map.unit}"
	...
	if (state.countTemp != null)
		state.countTemp++
	else
		state.countTemp=1
	if (map.value != device.currentTemperature)
		{
		if (timeElapsed >= minReportingTimeTemp)
			...
			if (state.countTemp > 1)
				map.descriptionText='Tx duplicates:' + state.countTemp + ' value '+ device.currentTemperature
			state.countTemp=0
			state.lastTemp = now()
			sendEvent(map)
			}
			...
			{         // queue the event
			map.descriptionText=new Date().format('yyyy-MM-dd HH:mm:ss')+' Tx:' + state.countTemp
			map.type = "delayed"
...
def humidityEvent( humidity, isDigital=false ) {
	if (settings?.logTempHumi) log.debug 'Humidity event ' +humidity
	 def map = [:]
	map.name = "humidity"
...
//    map.descriptionText = "${map.name} is ${Math.round((map.value) * 10) / 10} ${map.unit}"
	...
	if (map.value != device.currentHumidity)
		...
		if (timeElapsed >= minReportingTimeHumidity)
			...
			state.lastHumi = now()
			if (state.countHumi > 1)
					map.descriptionText='Tx duplicates:' + state.countHumi + ' value '+ device.currentHumidity
			state.countHumi=0
			sendEvent(map)
			}
		else
			{         // queue the event
			map.type = "delayed"
			map.descriptionText=new Date().format('yyyy-MM-dd HH:mm:ss')+' Tx:' + state.countHumi
			if (settings?.logEnable) log.debug "${device.displayName} DELAYING ${timeRamaining} seconds event : ${map}"
...
def updated() {
    ...
    if (getModelGroup() in ['TS0601_Tuya','TS0601_Haozee']) {
        Integer intValue = ((safeToDouble(settings?.temperatureSensitivity )) * 10) as int
        log.debug temperatureSensitivity +' intValue '+intValue+' temp as hex value '+zigbee.convertToHexString(intValue as int, 8) +' '+zigbee.convertToHexString(intValue as int, 16)
    ...
    if (getModelGroup() in ['TS0601_Haozee']) {
        Integer intValue = settings?.humiditySensitivity * 100 as int
        log.debug humiditySensitivity +' intValue '+intValue+' himidity as hex value '+zigbee.convertToHexString(intValue as int, 8) +' '+zigbee.convertToHexString(intValue as int, 16)
